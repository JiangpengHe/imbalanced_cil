...
class Inc_Learning_Appr:
    ...
    def train(self, t, trn_loader, val_loader, num_per_cls):
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        self.gradall = np.zeros(len(self.num_per_cls[t]), dtype = float)
        ...
        if t > 0:
            ...
            original_sample = []
            for i in range(t):
                temp_current = np.array(self.num_per_cls[self.taskidx])[idx:len(self.num_per_cls[i])]
                temp_og = np.array(self.num_per_cls[i])[idx:len(self.num_per_cls[i])]
                temp_diff = temp_og - temp_current
                removed_sample.extend(temp_diff.tolist())
                original_sample.extend(temp_og.tolist())
                idx = len(self.num_per_cls[i])
            removed_sample = np.array(removed_sample)
            self.kdprior = removed_sample
            self.kdprior_ratio = np.array(removed_sample) / np.array(original_sample)
            self.og_data = original_sample

            n_new = sum(self.num_per_cls[t][len(self.num_per_cls[t-1]):len(self.num_per_cls[t])])
            n_seen = sum(original_sample)
            # gamma to adjust the magnitude of attenutation factor
            gam = 1
            self.gamma = np.exp(-1*gam*(n_seen/(n_seen+n_new)))
            # lambda_b to control the influence of distillation part 
            lambda_b = 0.7
            self.kd_ratio = lambda_b * np.sqrt(n_seen/n_new)
    ...
    def calculate_each(self, outputs, targets):
        """Contains the main Task-Aware and Task-Agnostic metrics"""
