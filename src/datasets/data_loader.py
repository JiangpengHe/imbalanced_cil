...
def get_loaders(datasets, num_tasks, nc_first_task, batch_size, num_workers, pin_memory, exemplar, validation=.1):
    ...
    for idx_dataset, cur_dataset in enumerate(datasets, 0):
        ...
        # datasets
        trn_dset, val_dset, tst_dset, curtaskcla, num_per_cls_task = get_datasets(cur_dataset, dc['path'], num_tasks, nc_first_task,
                                                                validation=validation,
                                                                trn_transform=trn_transform,
                                                                tst_transform=tst_transform,
                                                                class_order=dc['class_order'], exemplar =  exemplar)
        ...
        for tt in range(num_tasks):
            ...
            tst_load.append(data.DataLoader(tst_dset[tt], batch_size=batch_size, shuffle=False, num_workers=num_workers,
                                            pin_memory=pin_memory))
    return trn_load, val_load, tst_load, taskcla, num_per_cls_task
...
def get_datasets(dataset, path, num_tasks, nc_first_task, validation, trn_transform, tst_transform, class_order=None, exemplar = None):
    ...
    if 'mnist' in dataset:
        ...
        # read data paths and compute splits -- path needs to have a train.txt and a test.txt with image-label pairs---
        # Note that in this case the train.txt should be in long-tailed version in advanve--------------
        all_data, taskcla, class_indices = basedat.get_data(path, num_tasks=num_tasks, nc_first_task=nc_first_task,
                                                            validation=validation, shuffle_classes=class_order is None,
    ...
    for task in range(num_tasks):
        temp_num_cls = get_num_per_class(all_data[task]['trn']['y'])
        if task > 0:
            temp = num_per_cls_task[task-1]
            temp2 = temp[:]
            for i in range(len(temp2)):
                if temp2[i] > exemplar:
                    temp2[i] = exemplar
            temp2.extend(temp_num_cls)
            num_per_cls_task.append(temp2)
        else:
            num_per_cls_task.append(temp_num_cls)
    ...
    return trn_dset, val_dset, tst_dset, taskcla, num_per_cls_task

def get_num_per_class(labels):
    temp_num_cls = np.zeros(len(np.unique(labels)), dtype = int)
    for i in labels:
        temp_num_cls[i] += 1
    return temp_num_cls.tolist()
