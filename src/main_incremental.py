...
def main(argv=None):
    ...
    num_exp_per_class = getattr(appr_exemplars_dataset_args, 'num_exemplars_per_class')
    ...
    # Loaders
    utils.seed_everything(seed=args.seed)
    trn_loader, val_loader, tst_loader, taskcla, num_per_cls_task = get_loaders(args.datasets, args.num_tasks, args.nc_first_task,
                                                              args.batch_size, num_workers=args.num_workers,
                                                              pin_memory=args.pin_memory, exemplar = num_exp_per_class)
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        if pod_flag:
            ...
            # add head to existing head
            net.add_head(taskcla[t][1])
            # add new head to entirely replace existing head
            #net.re_init_head(taskcla, t)
        #print (net.heads)
        net.to(device)
        ...
        # Train
        appr.train(t, trn_loader[t], val_loader[t], num_per_cls_task)
        print('-' * 100)
        ...
        for u in range(t + 1):
            ...
            #acc_each = appr.eval_each(u, tst_loader[u], u)
            #acc_each_all.extend(acc_each)
            if u < t:
